...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.action === "CONFIRM") {
      return Response.json({ policy: await confirmSuggestion(id, triggerType), applied: true });
    }
    if (body.action === "DISMISS") {
      return Response.json({ policy: await dismissSuggestion(id, triggerType), applied: true });
    }
    if (body.action === "REVERT") {
      return Response.json({ policy: await revertTopicReduction(id, triggerType), applied: true });
    }
