...
## 2026-09-12（第八轮）：执行收尾方案 E6（B1 曝光上报）

1. 曝光口径（近似，已明示）：列表加载即计一次曝光，feedbackType=EXPOSED 幂等去重；批量路由 `POST /interventions/exposures`（≤100 条，跨项目忽略）。
...
## 2026-09-12（第九轮）：执行收尾方案 E7（C1 证据可点开 + 统一回归）
...
| 项 | 代码验收 | 设备验收 | 总状态 |
|---|---|---|---|
| S1/M1/R1/F1/T1/B1/C1 | 全部 PASS | 尚未执行 | 全部 IN_PROGRESS |

设备矩阵（正常/空态/失败重试/重复点击/返回重进/大字号/深色/写入超时重进/重启追溯）尚未执行，任何一项不得标 VERIFIED。
